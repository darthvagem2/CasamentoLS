...
async function initializeStatusPage() {
    ...
    const guestIdRaw = params.get("guest");
    ...
    // normaliza o parâmetro guest (decodeURIComponent, trim, toLowerCase)
    function normalizeParam(value) {
        if (value == null) return "";
        let s = String(value);
        try { s = decodeURIComponent(s); } catch (e) { /* ignore malformed */ }
        return s.trim().toLowerCase();
    }

    const guestId = normalizeParam(guestIdRaw);

    const guests = await loadGuests();
    const guest = guests.find(item => normalizeParam(item.id || "") === guestId);
